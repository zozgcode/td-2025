...
export default function Header({ handleLogout, user }: any) {
  ...
  return (
    <div className="w-full border-b-2 border-b-[#008a00] min-h-[30px] relative flex items-center justify-between bg-[#ffffff] p-[20px] py-[15px]">
      <Image src="https://i.imgur.com/FUCIqQD.png" width={230} height={28} className="w-[200px]" alt="logo" />

      <div className="relative">
        <RiLogoutCircleLine className="text-2xl text-[#008a00]" onClick={toggleNav} />
        ...
        {open && (
          <div className="absolute mt-1 z-50 shadow bg-white border py-2 rounded-md right-0 flex flex-col justify-center gap-[5px]">
            ...
            <p className="text-[14px] m-1 px-[15px] text-center rounded-md py-[5px] bg-[#008a00] border whitespace-nowrap text-white" onClick={handleLogout}>
              Sign out
            </p>
          </div>
        )}
